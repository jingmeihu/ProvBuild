...
class Trial(AlchemyProxy):
    ...
    def pull_content(cls, tid, session=None):
        session = session or relational.session
        ttrial = cls.__table__
        result = session.query(ttrial).filter(ttrial.c.id == tid).one()
        return result

    def push_content(cls, id, res, session=None):
        session = session or relational.session
        ttrial = cls.__table__
        result = session.execute(
            ttrial.update()
            .values(script=res.script, code_hash=res.code_hash,docstring=res.docstring)
            .where(ttrial.c.id == id)
        )
        session.commit()

    def get_last_runupdate_id(cls, session=None):
        session = session or relational.session
        ttrial = cls.__table__
        result = session.query(ttrial).filter(ttrial.c.arguments == 'runupdate').order_by(ttrial.c.id.desc()).first()
        return result.id
