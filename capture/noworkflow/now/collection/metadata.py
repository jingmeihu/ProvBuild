...
class Metascript(object):                                                        # pylint: disable=too-many-instance-attributes
    ...
    def read_cmd_args_runupdate(self, args, cmd=None):
        """Read cmd line argument object for run-update"""
        #print(sys.argv[1:])
        if not cmd:
            cmd = " ".join(sys.argv[1:])
        print(cmd)
        self.command = cmd
        self.dir = os.path.dirname(args.script)
        self.argv = args.argv
        self.name = os.path.basename(args.argv[0])
        self.serialize = get_serializer(None)
        self.verbose = True
